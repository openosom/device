...
#define smdk2440_printf(format, ...)	\
    fprintf(stderr, "QEMU %s: " format, __FUNCTION__, ##__VA_ARGS__)

#define SMDK2440_GPIO_BACKLIGHT		S3C_GPG(4)
#define SMDK2440_GPIO_LCD_RESET		S3C_GPC(6)
#define SMDK2440_GPIO_nSD_DETECT	S3C_GPG(8)
#define SMDK2440_GPIO_WP_SD		S3C_GPH(8)
#define SMDK2440_GPIO_DM9000		S3C_GPF(7)
#define SMDK2440_GPIO_USB_PULLUP	S3C_GPC(5)

#define SMDK2440_IRQ_nSD_DETECT		S3C_EINT(16)
#define SMDK2440_IRQ_DM9000		S3C_EINT(7)
...
static ee24c08 * ee24c08_init(i2c_bus * bus)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (!dev) {
			smdk2440_printf("failed address %02x\n", 0x50+i);
		}
...
static void smdk2440_bl_switch(void *opaque, int line, int level)
{
	printf("QEMU: %s: LCD Backlight now %s (%d).\n", __FUNCTION__, level ? "on" : "off", level);
}

static void smdk2440_bl_intensity(int line, int level, void *opaque)
{
    struct smdk2440_board_s *s = (struct smdk2440_board_s *) opaque;
...
static void smdk2440_gpio_setup(struct smdk2440_board_s *s)
{
    ...
    s3c_gpio_out_set(s->cpu->io, SMDK2440_GPIO_BACKLIGHT,
                    *qemu_allocate_irqs(smdk2440_bl_switch, s, 1));

    s3c_timers_cmp_handler_set(s->cpu->timers, 1, smdk2440_bl_intensity, s);

    /* Register the SD card pins to the lower SD driver */
 	sd_set_cb(s->mmc,
 			s3c_gpio_in_get(s->cpu->io)[SMDK2440_GPIO_WP_SD],
			qemu_irq_invert(s3c_gpio_in_get(s->cpu->io)[SMDK2440_IRQ_nSD_DETECT]));
...
static int smdk2440_load_from_nand(NANDFlashState *nand,
		uint32_t nand_offset, uint32_t s3c_base_offset, uint32_t size)
{
	...
	for (page = 0; page < (size / 512); page++, src += 512 + 16, dst += 512) {
		if (nand_readraw(nand, nand_offset + src, buffer, 512)) {
			...
			smdk2440_printf("failed to load nand %d:%d\n",
...
static void smdk2440_reset(void *opaque)
{
    struct smdk2440_board_s *s = (struct smdk2440_board_s *) opaque;
    int32_t image_size;
	...
	if (s->boot_mode == BOOT_NAND) {
		 ...
		 */
		if (smdk2440_load_from_nand(s->nand, 0, S3C_RAM_BASE | 0x03f80000, 512*1024)> 0) {
			smdk2440_printf("loaded default u-boot from NAND\n");
			s->cpu->env->regs[15] = S3C_RAM_BASE | 0x03f80000; /* start address, u-boot already relocated */
		}
#if 0 && defined(LATER)
		if (smdk2440_load_from_nand(s->nand, 0, S3C_SRAM_BASE_NANDBOOT, S3C_SRAM_SIZE) > 0) {
			s->cpu->env->regs[15] = S3C_SRAM_BASE_NANDBOOT;	/* start address, u-boot relocating code */
			smdk2440_printf("4KB SteppingStone loaded from NAND\n");
		}
#endif
		 ...
		 */
		{
			...
			if (image_size > 0) {
				...
				smdk2440_printf("loaded override u-boot (size %x)\n", image_size);
				s->cpu->env->regs[15] = S3C_RAM_BASE | 0x03f80000;	/* start address, u-boot already relocated */
			}
		}
	}
	...
	if (s->kernel) {
	   	...
	   	if (image_size > 0) {
	   		if (image_size & (512 -1))	/* round size to a NAND block size */
	   			image_size = (image_size + 512) & ~(512-1);
	   		smdk2440_printf("loaded %s (size %x)\n", s->kernel, image_size);
	    }
	}
...
static const int smdk2440_ts_scale[6] = {
...
{
    struct smdk2440_board_s *mini =
    		(struct smdk2440_board_s *)qemu_mallocz(sizeof(struct smdk2440_board_s));
    ...
    if (cpu_model && strcmp(cpu_model, "arm920t")) {
    	smdk2440_printf("This platform requires an ARM920T core\n");
        exit(2);
    }
     ...
     */
    const char * boot_mode = getenv("SMDK2440_BOOT");

    if (boot_mode) {
    	if (!strcasecmp(boot_mode, "nor")) {
    		if (nor_idx < 0) {
    			printf("%s SMDK2440_BOOT(nor) error, no flash file specified", __func__);
    			abort();
    		} else
    		    mini->boot_mode = BOOT_NOR;
    	} else if (!strcasecmp(boot_mode, "nand")) {
    		if (nor_idx < 0) {
    			printf("%s SMDK2440_BOOT(nand) error, no flash file specified", __func__);
    			abort();
    		} else
    		    mini->boot_mode = BOOT_NOR;
    	} else
			printf("%s SMDK2440_BOOT(%s) ignored, invalid value", __func__, boot_mode);
    }
...

	{
		...
		if (strcmp(nd->model, "dm9000") == 0) {
			dm9000_init(nd, 0x20000000, 0x300, 0x304, s3c_gpio_in_get(mini->cpu->io)[SMDK2440_IRQ_DM9000]);
		}
	}

    s3c_adc_setscale(mini->cpu->adc, smdk2440_ts_scale);

    /* Setup initial (reset) machine state */
    qemu_register_reset(smdk2440_reset, mini);
