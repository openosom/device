/*
 * smdk2410 development board, support almost s3c2410 soc.
 *
...
static ee24c08 * ee24c08_init(i2c_bus * bus)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (!dev) {
			smdk2410_printf("failed address %02x\n", 0x50+i);
		}
...
static void smdk2410_bl_switch(void *opaque, int line, int level)
{
	printf("QEMU: %s: LCD Backlight now %s (%d).\n", __FUNCTION__, level ? "on" : "off", level);
}

static void smdk2410_bl_intensity(int line, int level, void *opaque)
{
    struct smdk2410_board_s *s = (struct smdk2410_board_s *) opaque;
...
static void smdk2410_gpio_setup(struct smdk2410_board_s *s)
{
    ...
    s3c_gpio_out_set(s->cpu->io, SMDK2410_GPIO_BACKLIGHT,
                    *qemu_allocate_irqs(smdk2410_bl_switch, s, 1));

    s3c_timers_cmp_handler_set(s->cpu->timers, 1, smdk2410_bl_intensity, s);

    /* Register the SD card pins to the lower SD driver */
 	sd_set_cb(s->mmc,
 			s3c_gpio_in_get(s->cpu->io)[SMDK2410_GPIO_WP_SD],
			qemu_irq_invert(s3c_gpio_in_get(s->cpu->io)[SMDK2410_IRQ_nSD_DETECT]));
...
static int smdk2410_load_from_nand(NANDFlashState *nand,
		uint32_t nand_offset, uint32_t s3c_base_offset, uint32_t size)
{
	...
	for (page = 0; page < (size / 512); page++, src += 512 + 16, dst += 512) {
		if (nand_readraw(nand, nand_offset + src, buffer, 512)) {
			...
			smdk2410_printf("failed to load nand %d:%d\n",
...
static void smdk2410_reset(void *opaque)
{
    struct smdk2410_board_s *s = (struct smdk2410_board_s *) opaque;
    int32_t image_size;
	...
	if (s->boot_mode == BOOT_NAND) {
		 ...
		 */
		if (smdk2410_load_from_nand(s->nand, 0, S3C_RAM_BASE | 0x03f80000, 512*1024)> 0) {
			smdk2410_printf("loaded default u-boot from NAND\n");
			s->cpu->env->regs[15] = S3C_RAM_BASE | 0x03f80000; /* start address, u-boot already relocated */
		}
#if 0 && defined(LATER)
		if (smdk2410_load_from_nand(s->nand, 0, S3C_SRAM_BASE_NANDBOOT, S3C_SRAM_SIZE) > 0) {
			s->cpu->env->regs[15] = S3C_SRAM_BASE_NANDBOOT;	/* start address, u-boot relocating code */
			smdk2410_printf("4KB SteppingStone loaded from NAND\n");
		}
#endif
		 ...
		 */
		{
			...
			if (image_size > 0) {
				...
				smdk2410_printf("loaded override u-boot (size %x)\n", image_size);
				s->cpu->env->regs[15] = S3C_RAM_BASE | 0x03f80000;	/* start address, u-boot already relocated */
			}
		}
	}
	...
	if (s->kernel) {
	   	...
	   	if (image_size > 0) {
	   		if (image_size & (512 -1))	/* round size to a NAND block size */
	   			image_size = (image_size + 512) & ~(512-1);
	   		smdk2410_printf("loaded %s (size %x)\n", s->kernel, image_size);
	    }
	}
...
static const int smdk2410_ts_scale[6] = {
...
{
    struct smdk2410_board_s *mini =
    		(struct smdk2410_board_s *)qemu_mallocz(sizeof(struct smdk2410_board_s));
    ...
    if (cpu_model && strcmp(cpu_model, "arm920t")) {
    	smdk2410_printf("This platform requires an ARM920T core\n");
        exit(2);
    }
    ...
    mini->cpu = s3c24xx_init(S3C_CPU_2410, 12000000 /* 12 mhz */, mini->ram, sram_base, mini->mmc);

    /* Setup peripherals */
    smdk2410_gpio_setup(mini);
...

	{
		...
		if (strcmp(nd->model, "dm9000") == 0) {
			dm9000_init(nd, 0x20000000, 0x300, 0x304, s3c_gpio_in_get(mini->cpu->io)[SMDK2410_IRQ_DM9000]);
		}
	}

    s3c_adc_setscale(mini->cpu->adc, smdk2410_ts_scale);

    /* Setup initial (reset) machine state */
    qemu_register_reset(smdk2410_reset, mini);
...
QEMUMachine smdk2410_machine = {
    "smdk2410",
    "Samsung SOC Dev-Boards (S3C2410A)",
    .init = smdk2410_init,
};
