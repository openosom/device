...
//		printf("zImage magic = 0x%08lx\n", *(ulong *)(to + 9*4));
	...
	/* Get machine type */
	mach_type = MACH_TYPE_SMDK2440;
//	printf("MACH_TYPE = %d\n", mach_type);
	char	*s;
	s = getenv ("machid");
	if (s) {
		mach_type = simple_strtoul (s, NULL, 10);
		printf ("Using mach_type 0x%x from environment\n", mach_type);
	}
...
{
	boot_zImage( 0x00080000,0x00500000);
	return 0;
}

U_BOOT_CMD(
	bootz,	3,	0,	do_boot_zImage,
	"bootz - boot Linux 's zImage\n",
	" - boot Linux 's zImage"
);
