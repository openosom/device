...
{
    struct neo_board_s *s = (struct neo_board_s *) opaque;
    s->cpu->nand->setwp(s->cpu->nand, level);
}
...
/* Board init.  */
static struct neo_board_s *neo1973_init_common(uint32_t cpu_id, int ram_size, DisplayState *ds,
                ...
                struct sd_card_s *mmc, uint32_t id)
{
    ...
    s->cpu = s3c24xx_init(cpu_id, s->ram, ds, s->mmc);
                ...
                const char *initrd_filename, const char *cpu_model)
{
    ...
    neo = neo1973_init_common(S3C_CPU_2410, ram_size, ds,
                    kernel_filename, cpu_model, sd, NEO1973_GTA01);

    neo->cpu->nand->reg(neo->cpu->nand, nand_init(NAND_MFR_SAMSUNG, 0x76));
}
                ...
                const char *initrd_filename, const char *cpu_model)
{
	...
	printf("Richard Nee : 1\n");
    neo = neo1973_init_common(S3C_CPU_2440, ram_size, ds,
                    kernel_filename, cpu_model, sd, NEO1973_GTA02F);
	printf("Richard Nee : 2\n");

    neo->cpu->nand->reg(neo->cpu->nand, nand_init(NAND_MFR_SAMSUNG, 0x76));
	printf("Richard Nee : 3\n");
    neo_gps_setup(neo);
	printf("Richard Nee : 4\n");
    neo_machid_init(neo);
	printf("Richard Nee : 5\n");
}
